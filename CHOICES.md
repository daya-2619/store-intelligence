...
### 3. PostgreSQL Bulk Upserts (Core) over ORM
We bypassed SQLAlchemy's standard ORM `db.add()` for event ingestion, opting instead for `insert().on_conflict_do_nothing()`. Handling 500 events per second via an ORM triggers N+1 lookup queries resulting in immediate CPU thrashing. Raw SQLAlchemy Core UPSERTs condensed 500 queries into a single atomic transaction.

### 4. Redis Caching
Real-time dashboards often suffer from "F5 Attacks", where multiple users repeatedly query the same complex aggregations. Wrapping our `pos_summary` and `live` endpoints in a simple Redis cache completely isolates the database from read-heavy dashboard spikes.
